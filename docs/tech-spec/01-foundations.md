...
> **Tinted-surface hue border (~40%, `66`):** every one of the six tint demo frames on the screen pairs the low-alpha **fill** with a **1px same-hue stroke at ~40% alpha** (`strokeWidth: 1`, `strokeAlignment: inner`) — verified on `WCTkf` nodes `JNawK`/`US4Cc` (`#ff6b3566`), `f41OQ`/`K3cb6G` (`#d9a73e66`), `AEhEs`/`Q5mRB` (`#5ae0a066`). The `66` suffix = 102/255 ≈ 40%. So a tinted surface is **fill `/12`(or `/18`) + border `/40` of the same token** (`border-{token}/40`), not a borderless block. This is distinct from the *solid*-token left-accent borders and from the CrossRef `/6` wash below.

> **Build rule:** never author raw hex tints — derive from a token via alpha. Canonical fill = **12%** (`bg-cat-*/12`, `bg-warning/12`, `bg-primary/12`); hover-lift = **18%**; the accompanying hue outline on a tinted surface = **40%** (`border-{token}/40`, the `66` suffix). Left-accent borders and pill text use the **solid** token (full opacity). When lifting camp-404's Badge/IconBadge (which default to `/15`), normalize to `/12`. The CrossRef callout primitive on this board uses a fainter `primary` wash (`#ff6b350f` ≈ 6%) with a 2px solid `$primary` left border — a one-off chrome accent, not part of the 12/18 fill convention.
...
- ✅ **Tint/alpha convention** — 12% fill / 18% hover-lift with exact 8-digit-hex encodings (`1f`/`2e`) across primary/warning/category, plus the ~40% (`66`) same-hue 1px tinted-surface border (`border-{token}/40`); solid-token rule; camp-404 `/15`→`/12` normalization.
...
- Tint convention encoded as 8-digit-hex alpha: 12% fill = suffix `1f` (31/255), 18% hover-lift = suffix `2e` (46/255), demonstrated for primary/warning/cat-travel. Each demo frame ALSO carries a 1px same-hue stroke at ~40% alpha = suffix `66` (102/255) — i.e. a tinted surface is fill `/12`(/`/18`) + border `/40` of the same token (`border-{token}/40`), not borderless (WCTkf JNawK/US4Cc/f41OQ/K3cb6G/AEhEs/Q5mRB). CrossRef uses a fainter ~6% wash (`0f`) — a one-off chrome accent outside the 12/18 fill convention. Camp-404 Badge/IconBadge default /15 must be normalized to /12.
